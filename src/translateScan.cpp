...
Scan2PointTranslator::Scan2PointTranslator() {
    scan_sub = nh.subscribe<sensor_msgs::LaserScan> ("/lidar_scan", 4, &Scan2PointTranslator::scanCallback, this);
    point_cloud_publisher = nh.advertise<sensor_msgs::PointCloud2>("/sync_scan_cloud_filtered", 4);
    // imu_publisher = nh.advertise<sensor_msgs::Imu>("/microstrain/imu", 10);
}

void Scan2PointTranslator::scanCallback( const sensor_msgs::LaserScan::ConstPtr &scan ) {
  ROS_INFO_STREAM("HERE with scan " << scan->header.seq);
  try {
